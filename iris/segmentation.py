...
segmentation_app = flask.Blueprint('segmentation', __name__)

@segmentation_app.route('/', methods=['GET'])
def index():
    image_id = flask.request.args.get('image_id', project.get_start_image())

    pprint(project['files'])

    return flask.render_template(
        'segmentation.html', image_id=image_id,
        image_shape=project['image_shape'], mask_area=project['segmentation']['mask_area'],
...
def merge_masks(image_id):
    ...
    for u, user_id in enumerate(users):
        ...
        mask.score_pending = len(users) == 1

    total_agreement = \
        np.take_along_axis(counter, final_mask[..., np.newaxis], axis=-1).sum()
    image.segmentation_agreement = total_agreement / counter.sum()
    db.session.commit()

    # Retranslate to original classes:
    final_mask = np.vectorize(classes.__getitem__, otypes=[np.uint8])(final_mask)
...
def load_mask(image_id):
    ...
    if user_id is None:
        return flask.make_response('No user login detected!', 403)
...
def save_mask(image_id):
    ...
    if user_id is None:
        return flask.make_response("No user login found!", 403)
